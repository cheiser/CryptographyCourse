...
public class CryptoLibExtended {
	...
	/**
	 * Finds the resulting x when we want to find to solve a problem like <br />
	 * x == 2 mod 3 <br />
	 * 		x == 2 mod 4 <br />
	 * 			x == 1 mod 5
	 * 
	 ...
	 * problem.
	 */
	public static BigInteger[] chineseRemainderTheoremBI(BigInteger[] mods, BigInteger[] equalMods){
		...
		for(int i = 0; i < newMods.length; i++){
			newMods[i] = new BigInteger("1");
			result[1] = result[1].multiply(mods[i]); // result[1] * mods[i];
			for(int j = 0; j < newMods.length; j++){
				if(i != j){
					newMods[i] = newMods[i].multiply(mods[j]);// newMods[i] * mods[j];
				}
			}
		}
		...
		for(int i = 0; i < newMods.length; i++){
			if(newMods[i].mod(mods[i]).compareTo(equalMods[i]) != 0){
				BigInteger modInv = ModInvBI(newMods[i], mods[i]); // BI already got a built-in modinv...
				newMods[i] = newMods[i].multiply(modInv); // newMods[i] * modInv;
				newMods[i] = newMods[i].multiply(equalMods[i]);// newMods[i] * equalMods[i];
			}
			sum = sum.add(newMods[i]); // sum += newMods[i];
		}
		
		
		result[0] = sum.mod(result[1]);// sum % result[1];
